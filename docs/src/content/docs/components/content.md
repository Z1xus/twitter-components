...
Post text and links. Text is escaped before rendering.

## Data

Assign a `TwitterContent` object to `.data`. Assign display settings to `.options`. Reassign the object to update the component.

```js
import 'twitter-components/register'
const element = document.querySelector('twitter-content')
element.data = data
